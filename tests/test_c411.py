...
class TestCategoryMapping:
    ...
    def test_emission_reality(self):
        c = C411(_config())
        cat, sub = c._get_category_subcategory({'category': 'TV', 'genres': 'Reality'})
        assert cat == 1
        assert sub == 5  # Emission TV

    def test_emission_talk(self):
        c = C411(_config())
        cat, sub = c._get_category_subcategory({'category': 'TV', 'genres': 'Talk'})
        assert cat == 1
        assert sub == 5  # Emission TV

    def test_emission_news(self):
        c = C411(_config())
        cat, sub = c._get_category_subcategory({'category': 'TV', 'genres': 'News'})
        assert cat == 1
        assert sub == 5  # Emission TV

    def test_soap_is_serie_not_emission(self):
        # Soaps (feuilletons quotidiens) sont des séries scénarisées, pas des émissions
        c = C411(_config())
        cat, sub = c._get_category_subcategory({'category': 'TV', 'genres': 'Soap'})
        assert cat == 1
        assert sub == 7  # Série TV

    def test_soap_drama_is_serie(self):
        c = C411(_config())
        cat, sub = c._get_category_subcategory({'category': 'TV', 'genres': 'Soap, Drama'})
        assert cat == 1
        assert sub == 7  # Série TV
