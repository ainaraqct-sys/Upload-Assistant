...
class TestCategory:
    ...
    def test_emission_reality(self):
        t = TORR9(_config())
        cat, sub = t._get_category(_meta_base(category='TV', genres='Reality'))
        assert cat == 'Séries'
        assert sub == 'Emission TV'

    def test_emission_talk(self):
        t = TORR9(_config())
        cat, sub = t._get_category(_meta_base(category='TV', genres='Talk'))
        assert cat == 'Séries'
        assert sub == 'Emission TV'

    def test_emission_news(self):
        t = TORR9(_config())
        cat, sub = t._get_category(_meta_base(category='TV', genres='News'))
        assert cat == 'Séries'
        assert sub == 'Emission TV'
    ...
    def test_soap_is_serie_not_emission(self):
        ...
        t = TORR9(_config())
        cat, sub = t._get_category(_meta_base(category='TV', genres='Soap'))
        assert cat == 'Séries'
        assert sub == 'Séries TV'

    def test_soap_drama_is_serie(self):
        t = TORR9(_config())
        cat, sub = t._get_category(_meta_base(category='TV', genres='Soap, Drama'))
        assert cat == 'Séries'
        assert sub == 'Séries TV'
