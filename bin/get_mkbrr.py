...
MKBRR_VERSION = "v1.18.0"  # renovate: datasource=github-releases depName=autobrr/mkbrr


def _fetch_latest_github_release(repo: str, fallback: str) -> str:
    """Fetch the latest release tag from GitHub API, falling back to `fallback` on error."""
    import json
...
class MkbrrBinaryManager:
    @staticmethod
    async def ensure_mkbrr_binary(base_dir: Union[str, Path], debug: bool, version: Optional[str] = None) -> str:
        if version is None:
            version = _fetch_latest_github_release("autobrr/mkbrr", MKBRR_VERSION)
    ...
    @staticmethod
    def download_mkbrr_for_docker(base_dir: Union[str, Path] = ".", version: Optional[str] = None) -> str:
