...
C411 (`c411.org`) is a French private tracker using a custom API (not UNIT3D).

#### Authentication
- `api_key` (str, required): API key from your c411.org profile settings.
- `announce_url` (str, required): Announce URL with your passkey — found on your profile page.

#### Upload behaviour
- `include_screenshots` (bool): Include screenshots in the upload description. Default: `False`.
- `draft` (bool): Save the upload as a draft instead of publishing it immediately. Equivalent to `--draft` / `-dr` on the CLI. Max 15 drafts per user on c411.org. Default: `False`.
- `exclusive` (bool): Mark the upload as exclusive on c411.org (adds `isExclusive=true` to the upload). Default: `False`.

#### Description customisation
- `group_name` (str): Your group name — displayed above the title as "*NAME* présente".
- `group_logo_url` (str): URL of your group logo image — displayed at the top of the description, above the title. Must be a direct image URL (not a page URL).
- `group_banner_url` (str): URL of your group banner image — displayed in the signature at the bottom of the description. Must be a direct image URL hosted on an external image host (imgbb, imgbox, etc.) — c411.org self-referencing URLs are blocked.
- `group_quote` (str): Group motto / quote — displayed in the signature below the banner.
- `custom_warning` (str): BBCode warning banner displayed at the very top of the description in red (optional).
  Example: `"PLEASE CHECK YOUR DEVICE COMPATIBILITY BEFORE DOWNLOADING!"`
- `custom_message` (str): Custom message displayed at the bottom in a "Notes" section (BBCode for the description, plain text for the NFO). Optional.
  Example: `"[b]Enjoy the movie![/b]"`

#### NFO customisation
The NFO is a plain-text file generated automatically. You can brand it with your group's ASCII art.

- `nfo_ascii_art` (str): ASCII art for the NFO header (top of the file). Use `\n` as line separator between rows. All rows must have the same leading whitespace so that multi-row block fonts align correctly.
- `nfo_team_art` (str): ASCII art for the "Team" section at the bottom of the NFO. Same format as `nfo_ascii_art`. Can be the same value or a different wider logo.
