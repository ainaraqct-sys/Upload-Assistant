        ...
        if meta["category"] == "TV":
            ...
            if not meta.get("anime"):
                try:
                    ...
                    se_match = re.search(r"S\d{1,4}E\d{1,3}", video, re.IGNORECASE)
                    # TODO: re-enable once C411 year validation accepts DD.MM.YYYY dates in names
                    # if daily_match and se_match and not meta.get("manual_date"):
                    #     raw_date = daily_match.group().replace(".", "-")  # YYYY-MM-DD
                    #     parts = raw_date.split("-")
                    #     if len(parts) == 3:
                    #         meta["airdate_in_name"] = f"{parts[2]}.{parts[1]}.{parts[0]}"
                    #     else:
                    #         meta["airdate_in_name"] = raw_date
                    # Only treat as daily if: user explicitly passed --daily, OR the filename has a date but no S##E## pattern
                    if (meta.get("manual_date") or (daily_match and not se_match)) and not meta.get("manual_season"):
