...
class NameManager:
    ...
    async def get_name(self, meta: Meta) -> tuple[str, str, str, list[str]]:
        active_trackers: list[str] = [tracker for tracker in TRACKER_DISC_REQUIREMENTS if tracker in meta.get("trackers", [])]
        if active_trackers:
        ...
        if meta["category"] == "MOVIE":  # MOVIE SPECIFIC
            ...
            if type == "DISC":  # Disk
                if meta["is_disc"] == "BDMV":
                    name = (
                        f"{title} {alt_title} {year} {season}{episode} {three_d} {edition} {hybrid} {repack} {resolution} {region} {uhd} {source} {hdr} {video_codec} {audio}"
                    )
                    potential_missing = ["edition", "region", "distributor"]
                if meta["is_disc"] == "DVD":
    ...
    async def extract_title_and_year(self, meta: Meta, filename: str) -> tuple[Optional[str], Optional[str], Optional[str]]:
        ...
        season_pattern = r"\bS(\d{1,4})\b"
        season_episode_pattern = r"\bS(\d{1,4})E(\d{1,3})\b"
