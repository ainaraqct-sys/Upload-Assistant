...
FRENCH_FORCED_VALUES: frozenset[str] = {
    "forced",
    "forcés"
}

FRENCH_FULL_VALUES: frozenset[str] = {
    "full",
    "complets",
    "cc",
    "sdh",
...
AUDIODESC_PATTERNS: frozenset[str] = {
    r"\baudio\s*description\b",
    r"\baudio-description\b",
    r"\baudiodescription\b",
    r"\bad\b",
}
...
class FrenchTrackerMixin:
    ...
    # Whether to strip the French dub suffix (VFF, VFQ, VF2, …) from the language tag
    # in the release name, keeping only the base tag (e.g. MULTI.VFF → MULTI, VFF → "").
    # Set to True for trackers that do not want the regional dub variant in the name.
    STRIP_FRENCH_DUB_SUFFIX: bool = False

    # Whether to include the audio codec tag (DDP 5.1, TrueHD, …) in the release name.
    # Set to False for trackers that do not want audio in the release name.
    INCLUDE_AUDIO_IN_NAME: bool = True
    ...
    def _has_french_subs(meta: Meta) -> bool:
        """Check whether complete French subtitles are present in MediaInfo.

        Forced subtitle tracks are excluded: they only cover a handful of
        foreign-language lines and do not allow a French speaker to follow
        the full original-language audio track.
        """
        if "mediainfo" not in meta or "media" not in meta.get("mediainfo", {}):
            return False
        for track in meta["mediainfo"]["media"].get("track", []):
            if track.get("@type") != "Text":
                continue
            # Skip forced subtitle tracks — not a complete subtitle track
            if str(track.get("Forced", "")).strip().lower() == "yes":
                continue
            # Skip forced subtitle tracks by its title/name (not only the flag) — not a complete subtitle track
            if any(word in str(track.get("Title", "")).strip().lower() for word in FRENCH_FORCED_VALUES):
                continue
    ...
    async def _check_french_lang_dupes(
        ...
        """
        # ── Internal team bypass ──
        # If the upload is from an internal team (THESYNDiCATE), only block on
        # dupes from the same team. External releases don't count as dupes.
        if dupes:
            upload_group = self._get_release_group(meta).upper()
            if upload_group in {"THESYNDICATE"}:
                before = len(dupes)
                dupes = [d for d in dupes if upload_group in (d.get("name", "") if isinstance(d, dict) else str(d)).upper()]
                if meta.get("debug") and len(dupes) < before:
                    console.print(f"[cyan]Internal team bypass: {before - len(dupes)} external dupe(s) ignored (upload is {upload_group})[/cyan]")
            if not dupes:
                return dupes
        ...
        # ── Codec coexistence ──
        # AV1 and H265/x265 are distinct codecs and can coexist on the same tracker.
        # If the upload is AV1, drop H265/x265 dupes (and vice versa).
        if dupes:
            def _get_codec(name: str) -> str:
                n = name.upper()
                if re.search(r"\bAV1\b", n):
                    return "AV1"
                if re.search(r"\b(H265|X265|HEVC)\b", n):
                    return "H265"
                if re.search(r"\b(H264|X264|AVC)\b", n):
                    return "H264"
                return ""

            upload_path = str(meta.get("path", meta.get("uuid", "")))
            upload_codec = _get_codec(upload_path)
            if upload_codec:
                before = len(dupes)
                dupes = [
                    d for d in dupes
                    if not _get_codec(d.get("name", "") if isinstance(d, dict) else str(d))
                    or _get_codec(d.get("name", "") if isinstance(d, dict) else str(d)) == upload_codec
                ]
                if meta.get("debug") and len(dupes) < before:
                    console.print(f"[cyan]Codec coexistence: {before - len(dupes)} dupe(s) removed (upload={upload_codec})[/cyan]")
    ...
    async def _build_audio_string(self, meta: Meta) -> str:
        ...
        def _fr_precision() -> str:
            ...
            if is_vf2_filename:
                return "VF2"
            # VFQ takes priority over VOF: a Quebec track is VFQ even on originally-French content
        ...
        # ── MULTi — non-French track present alongside French ──
        # MULTI requires a non-French audio track (e.g. English + French).
        # Multiple tracks of the same French variant (2x VFQ, 2x VFF) are NOT MULTI.
        # MULTI also requires FR subtitles: without them, the VO track is unusable
        # for a French speaker, making the release effectively VFF-only.
        ...
        # ── Single French track (or multiple same-variant French tracks) ──
        return _fr_precision()
    ...
    async def get_name(self, meta: Meta) -> dict[str, str]:
        ...
        # For originally-French TV series, guard against TMDb returning a
        # city/qualifier-suffixed series title (e.g. "Meurtres à Bayonne" for
        # the anthology series "Meurtres à...").  If the filename already
        # carries a French title that is a clean prefix of the TMDb French
        # title, prefer the shorter filename-derived title.
        if is_original_french and meta.get("category", "MOVIE") == "TV" and title:
            uuid = str(meta.get("uuid", ""))
            # Extract the title portion of the filename (everything before SxxExx)
            m = re.match(r"^(.+?)\.S\d{2}(?:E\d{2,4})?[\.\s]", uuid, re.IGNORECASE)
            if m:
                file_title = m.group(1).replace(".", " ").strip()
                # Normalise both to lowercase, no accents for comparison
                def _norm(s: str) -> str:
                    import unicodedata
                ...
                fn = _norm(file_title)
                ft = _norm(title)
                # If the TMDb French title starts with the filename title and
                # adds at least one extra word, the TMDb title is over-specific
                # (e.g. anthology with city suffix). Use the filename title.
                if ft.startswith(fn) and len(ft) > len(fn) + 1:
                    title = file_title
        language = await self._build_audio_string(meta)
        if self.STRIP_FRENCH_DUB_SUFFIX:
            language = re.sub(r"\.?VFF$", "", language)
        ...
        if resolution == "OTHER":
            resolution = ""
        audio = meta.get("audio", "").replace("Dual-Audio", "").replace("Dubbed", "").replace("DD+", "DDP") if self.INCLUDE_AUDIO_IN_NAME else ""
        ...
        doc_tag = "DOC" if "documentary" in genres_str else ""
        ...
        if category == "MOVIE":
            if type_val == "DISC":
                ...
                if disc == "BDMV":
                    name = f"{title} {year} {three_d} {edition} {repack} {doc_tag} {language} {resolution} {hybrid} {region} {uhd} {source} {hdr} {audio} {video_codec}"
                elif disc == "DVD":
                    name = f"{title} {year} {edition} {repack} {doc_tag} {language} {region} {source} {dvd_size} {audio}"
                elif disc == "HDDVD":
                    name = f"{title} {year} {edition} {repack} {doc_tag} {language} {resolution} {source} {audio} {video_codec}"
            elif type_val == "REMUX" and source in ("BluRay", "HDDVD"):
                name = f"{title} {year} {three_d} {edition} {repack} {doc_tag} {language} {resolution} {hybrid} {uhd} {source} REMUX {hdr} {audio} {video_codec}"
            elif type_val == "REMUX" and source in ("PAL DVD", "NTSC DVD", "DVD"):
                name = f"{title} {year} {edition} {repack} {doc_tag} {language} {source} REMUX {audio}"
            elif type_val == "REMUX":
                name = f"{title} {year} {edition} {repack} {doc_tag} {language} {resolution} {hybrid} {uhd} {source} REMUX {hdr} {audio} {video_codec}"
            elif type_val == "ENCODE":
                name = f"{title} {year} {edition} {repack} {doc_tag} {language} {resolution} {hybrid} {uhd} {source} {hdr} {audio} {video_encode}"
            elif type_val == "WEBDL":
                name = f"{title} {year} {edition} {repack} {doc_tag} {language} {resolution} {hybrid} {uhd} {service} {web_lbl} {hdr} {audio} {video_encode}"
            elif type_val == "WEBRIP":
                name = f"{title} {year} {edition} {repack} {doc_tag} {language} {resolution} {hybrid} {uhd} {service} WEBRip {hdr} {audio} {video_encode}"
            elif type_val == "HDTV":
                name = f"{title} {year} {edition} {repack} {doc_tag} {language} {resolution} {source} {audio} {video_encode}"
            elif type_val == "DVDRIP":
                name = f"{title} {year} {repack} {doc_tag} {language} {source} DVDRip {audio} {video_encode}"

        # ── TV ──
        elif category == "TV":
            # TODO: re-enable once C411 year validation accepts DD.MM.YYYY dates in names
            # airdate = meta.get("airdate_in_name", "")
            # se = f"{season}{episode}"
            # se_date = f"{se} {airdate}".strip() if airdate else se
            se = f"{season}{episode}"
            if type_val == "DISC":
                disc = meta.get("is_disc", "")
                if disc == "BDMV":
                    name = f"{title} {year} {se} {three_d} {edition} {repack} {doc_tag} {language} {resolution} {hybrid} {region} {uhd} {source} {hdr} {audio} {video_codec}"
                elif disc == "DVD":
                    name = f"{title} {year} {se} {three_d} {edition} {repack} {doc_tag} {language} {region} {source} {dvd_size} {audio}"
                elif disc == "HDDVD":
                    name = f"{title} {year} {se} {edition} {repack} {doc_tag} {language} {resolution} {source} {audio} {video_codec}"
            elif type_val == "REMUX" and source in ("BluRay", "HDDVD"):
                name = f"{title} {year} {se} {part} {three_d} {edition} {repack} {doc_tag} {language} {resolution} {hybrid} {uhd} {source} REMUX {hdr} {audio} {video_codec}"
            elif type_val == "REMUX" and source in ("PAL DVD", "NTSC DVD", "DVD"):
                name = f"{title} {year} {se} {part} {edition} {repack} {doc_tag} {language} {source} REMUX {audio}"
            elif type_val == "REMUX":
                name = f"{title} {year} {se} {part} {edition} {repack} {doc_tag} {language} {resolution} {hybrid} {uhd} {source} REMUX {hdr} {audio} {video_codec}"
            elif type_val == "ENCODE":
                name = f"{title} {year} {se} {part} {edition} {repack} {doc_tag} {language} {resolution} {hybrid} {uhd} {source} {hdr} {audio} {video_encode}"
            elif type_val == "WEBDL":
                name = f"{title} {year} {se} {part} {edition} {repack} {doc_tag} {language} {resolution} {hybrid} {uhd} {service} {web_lbl} {hdr} {audio} {video_encode}"
            elif type_val == "WEBRIP":
                name = f"{title} {year} {se} {part} {edition} {repack} {doc_tag} {language} {resolution} {hybrid} {uhd} {service} WEBRip {hdr} {audio} {video_encode}"
            elif type_val == "HDTV":
                name = f"{title} {year} {se} {part} {edition} {repack} {doc_tag} {language} {resolution} {source} {audio} {video_encode}"
            elif type_val == "DVDRIP":
                name = f"{title} {year} {se} {repack} {doc_tag} {language} {source} DVDRip {audio} {video_encode}"
    ...
    def _format_audio_bbcode(self, mi_text: str, meta: Optional[Meta] = None) -> list[str]:
        ...
        for i, at in enumerate(tracks):
            ...
            is_audio_desc = any(re.search(pattern, title_norm) for pattern in AUDIODESC_PATTERNS)
    ...
    def _format_subtitle_bbcode(self, mi_text: str, meta: Optional[Meta] = None) -> list[str]:
        ...
        for i, st in enumerate(tracks):
            ...
            # Detect wrong forced flag
            if forced and any(word in str(title).strip().lower() for word in FRENCH_FULL_VALUES):
                forced = False
