...
class UNIT3D:
    ...
    def _set_skip_reason(self, meta: dict[str, Any], reason: str) -> None:
        """Store a human-readable skip reason that will appear in Discord notifications."""
        self._skip_reason = reason
        meta.setdefault("tracker_status", {}).setdefault(self.tracker, {})["skip_reason"] = reason
    ...
    async def search_existing(self, meta: dict[str, Any], _: Any) -> list[dict[str, Any]]:
        ...
        if not self.api_key:
            ...
            meta["tracker_status"][self.tracker]["skip_reason"] = "API key manquante"
            meta["skipping"] = f"{self.tracker}"
            return dupes

        should_continue = await self.get_additional_checks(meta)
        if not should_continue:
            # If no specific reason was set by get_additional_checks, use a generic one
            if not meta["tracker_status"][self.tracker].get("skip_reason"):
                meta["tracker_status"][self.tracker]["skip_reason"] = "Conditions du tracker non remplies"
            meta["skipping"] = f"{self.tracker}"
            return dupes
    ...
    async def get_personal_release(self, meta: dict[str, Any]) -> dict[str, str]:
        personal_release = "1" if meta.get("personalrelease", False) or self.tracker_config.get("personalrelease", False) else "0"
