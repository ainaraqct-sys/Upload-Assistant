...
def _print_config_error(
    error_type: str, message: str, lineno: Optional[int] = None, text: Optional[str] = None, offset: Optional[int] = None, suggestion: Optional[str] = None
) -> None:
...
if os.path.exists(_config_path):
    try:
        ...
        if discord_config is not None:
            ...
            _wh = discord_config.get("discord_webhook_url", "")
            if isinstance(_wh, str) and _wh.strip():
                discord_webhook_url = _wh.strip()
    except SyntaxError as e:
        _print_config_error("Syntax error", str(e.msg) if e.msg else "Invalid syntax", lineno=e.lineno, text=e.text, offset=e.offset)
...
async def process_meta(meta: Meta, base_dir: str, bot: Any = None) -> None:
    ...
    if use_discord and (bot or discord_webhook_url) and not meta.get("debug", False):
        _trackers = meta.get("trackers", [])
        _trackers_str = ", ".join(_trackers) if _trackers else "unknown"
        await DiscordNotifier.send_discord_notification(config, bot, f"Starting upload process for: {Path(meta['path']).name} → [{_trackers_str}]", debug=False, meta=meta, webhook_url=discord_webhook_url)
    ...
    if meta.get("emby_debug", False):
        ...
        if "matched_tracker" not in meta:
            ...
            await tracker_data_manager.get_tracker_data(meta["video"], meta, search_term, search_file_folder, meta["category"], only_id=meta["only_id"])
    ...
    while confirm is False:
        ...
        meta, _help, _before_args = cast(tuple[Meta, Any, Any], parser.parse(list(" ".join(sys.argv[1:]).split(" ")) + list(editargs_tracking), meta))
            ...
            if meta.get("comparison", False):
                    ...
                    if meta["is_disc"] == "BDMV":
                        ...
                        try:
                            await takescreens_manager.disc_screenshots(
                                meta, bdmv_filename, bdinfo, meta["uuid"], base_dir, use_vs, meta.get("image_list", []), meta.get("ffdebug", False), 0
                            )
                ...
                if "image_list" in meta and meta["image_list"]:
                    try:
                        ...
                        image_data = {"image_list": image_list, "image_sizes": meta.get("image_sizes", {}), "tonemapped": meta.get("tonemapped", False)}
...
async def update_notification(base_dir: str) -> Optional[str]:
    ...
    if version.parse(remote_version) > version.parse(local_version):
        console.print(f"[red][NOTICE] [green]Update available: {'v' if not remote_version.startswith('v') else ''}[/green][yellow]{remote_version}")
        console.print(f"[red][NOTICE] [green]Current version: {'v' if not local_version.startswith('v') else ''}[/green][yellow]{local_version}")
...
async def do_the_thing(base_dir: str) -> None:
    ...
    try:
        ...
        for queue_item in queue_list:
            ...
            # Only init the legacy bot if no webhook URL is configured
            if (
                use_discord
                and not discord_webhook_url
            ...
            if "we_are_uploading" not in meta or not meta.get("we_are_uploading", False):
                ...
                if successful_trackers < skip_uploading_int and not meta["debug"]:
                    console.print(f"[red]Not enough successful trackers ({successful_trackers}/{skip_uploading_int}). No uploads being processed.[/red]")
                    if use_discord and (bot or discord_webhook_url) and not meta.get("debug", False):
                        await DiscordNotifier.send_upload_status_notification(config, bot, meta, webhook_url=discord_webhook_url)
                else:
                    ...
                    if use_discord and (bot or discord_webhook_url) and not meta.get("debug", False):
                        # Enrich tracker_status with full torrent URL for Discord embeds
                        for _tk, _st in cast(dict[str, Any], meta.get("tracker_status", {})).items():
                            if "torrent_id" in _st and _tk in tracker_class_map and "torrent_url" not in _st:
                                try:
                                    _st["torrent_url"] = tracker_class_map[_tk](config=config).torrent_url + str(_st["torrent_id"])
                                except Exception:
                                    pass
                        await DiscordNotifier.send_upload_status_notification(config, bot, meta, webhook_url=discord_webhook_url)
                    ...
                    if "queue" in meta and meta.get("queue") is not None:
                        ...
                        if "limit_queue" in meta and int(meta["limit_queue"]) > 0:
                            console.print(
                                f"[cyan]Successfully uploaded {processed_files_count - skipped_files_count} of {meta['limit_queue']} in limit with {total_files} files."
                            )
                        else:
            ...
            # Upload status already sent via send_upload_status_notification above (rich embeds)
...
async def process_cross_seeds(meta: Meta) -> None:
    ...
    dupe_checked_trackers = [t for t in cast(list[Any], meta.get("dupe_checked_trackers", [])) if isinstance(t, str)]
...
async def get_mkbrr_path(meta: Meta, base_dir: Optional[str] = None) -> Optional[str]:
    try:
        ...
        mkbrr_path = await MkbrrBinaryManager.ensure_mkbrr_binary(resolved_base_dir, debug=meta["debug"])
...
def _auto_update_binaries() -> None:
    """Background thread: silently keep binaries up to date at startup.

    Skipped inside Docker containers (binaries are baked into the image).
    Never raises — a failed update is silently ignored.
    """
    if os.path.exists("/.dockerenv"):
        return
    try:
        from bin.MI.get_mi import download_dvd_mediainfo
        download_dvd_mediainfo(base_dir, debug=False)
    except Exception:
        pass
...
if __name__ == "__main__":
    ...
    # Start binary auto-update in the background (non-blocking)
    threading.Thread(target=_auto_update_binaries, daemon=True).start()
