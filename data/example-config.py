...
config = {
    ...
    "TRACKERS": {
        ...
        # Available tracker: A4K, ACM, AITHER, ANT, AR, ASC, AZ, BHD, BHDTV, BJS, BLU, BT, C411, CBR, CZ, DC, DP, DT, EMUW, FF, FL, FNP, FRIKI, GPW, HDB, HDS, HDT, HHD, HUNO, IHD, IS, ITT, LACALE, LCD, LDU, LST, LT, LUME, MTV, NBL, OE, OTW, PHD, PT, PTER, PTP, PTS, PTT, R4E, RAS, RF, RTF, SAM, SHRI, SN, SP, SPD, STC, THR, TIK, TL, TLZ, TORR9, TOS, TTG, TTR, TVC, ULCX, UTP, YOINK, YUS
        ...
        "C411": {
            ...
            # Your group name — displayed above the title as "NAME présente"
            "group_name": "",
            # URL of your group logo image — displayed at the top of the description, above the title
            # Must be a direct image URL (not a page URL)
            "group_logo_url": "",
            # URL of your group banner image — displayed in the signature at the bottom of the description
            # Must be a direct image URL on an external host (imgbb, imgbox, etc.)
            # c411.org self-referencing URLs are blocked for inline images
            "group_banner_url": "",
            # Group motto / quote — displayed in the signature below the banner
            "group_quote": "",
            # BBCode warning banner displayed at the very top of the description in red (optional)
            # Example: "PLEASE CHECK YOUR DEVICE COMPATIBILITY BEFORE DOWNLOADING!"
            "custom_warning": "",
            # Custom message displayed at the bottom in a "Notes" section (optional)
            # Used as BBCode in the description and as plain text in the NFO
            # Example: "[b]Enjoy![/b]"
            "custom_message": "",
            # --- NFO customisation ---
            # ASCII art for the NFO header — use \n as line separator between rows
            # All rows must have the same leading whitespace so multi-row block fonts align correctly
            "nfo_ascii_art": "",
            # ASCII art for the "Team" section at the bottom of the NFO — same format as nfo_ascii_art
            # Example: "* ___ _  _ ____    ____ *\n*  |  |__| |___    [__  *"
            "nfo_team_art": "",
            # Save uploads as drafts on c411.org (max 15 drafts per user)
            # Can also be triggered with the --draft / -dr CLI flag
            "draft": False,
            # Mark the upload as exclusive on c411.org
            "exclusive": False,
        },
        "CZ": {
            # Instead of using the tracker acronym for folder name when sym/hard linking, you can use a custom name
        ...
        "LACALE": {
            ...
            # API key — generate one in your La Cale account settings
            "api_key": "",
            # Announce URL with your passkey — found in your La Cale profile
            "announce_url": "",
            # Upload anonymously
            "anon": False,
        },
        ...
        "TOS": {
            ...
            "anon": False,
            # Your TOS username (used in "Autres releases" description link)
            "username": "",
            # Upload with Exclusive flag (team of staff only)
            "exclusive": False,
            # Mark upload as a personal release
    ...
    "DISCORD": {
        ...
        "use_discord": False,
        # Set to True to only send notifications in unattended mode (-ua)
        "only_unattended": True,
        # Set to True to send the tracker torrent urls in the notification
        "send_upload_links": True,
        # --- Webhook (recommended) ---
        # Create a webhook in your Discord channel settings and paste the URL here.
        # When set, no bot token or channel ID is needed.
        "discord_webhook_url": "",
        # --- Legacy bot (optional, ignored if webhook URL is set) ---
        "discord_bot_token": "",
        "discord_channel_id": "",
